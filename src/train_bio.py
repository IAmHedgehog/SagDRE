...
def train(opt, data_opt):
    ...
    for epoch in range(start_epoch, opt.epoch + 1):
        ...
        for d in tqdm(train_loader, desc='Epoch '+str(epoch), unit='b'):
            ...
            loss2 = Aloss(
                m_preds.view(-1, 2), m_label.view(-1, 2), r_mask.view(-1))
            loss1 = torch.sum(EntropyLoss(
                m_preds.view(-1, 2), b_label.view(-1)) * r_mask.view(-1)) / (
