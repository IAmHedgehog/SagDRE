...
class BERTDGLREDataset(IterableDataset):
    ...
    def process_doc(self, doc, dataset, ner2id, bert):
        ...
        labels = []
        for cur_l in doc.get('labels', []):
            labels.append(cur_l)
            labels.append({'h': cur_l['t'], 't': cur_l['h'], 'r': cur_l['r']})
            ...
            for i, example in enumerate(minibatch):
                ...
                if self.dataset_type == 'train':
                    ...
                    if self.negativa_alpha > 0.0:
                        ...
                        # lower_bound = int(max(10, len(train_tripe) * self.negativa_alpha))
                        lower_bound = int(len(train_tripe) * self.negativa_alpha)
